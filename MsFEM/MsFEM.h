...
namespace MsFEM {
	...
	template <typename Dirichlet, typename Neumann>
	void MsFEM_forElasticDeformation_OrderBF1_forPoly(
		bool is_print_logFile, //input
		double MIN_RESIDUAL,
		char* fine_mesh_directory,
		math::SimpleGrid& geo_grid, //input
...
#pragma omp parallel for schedule(dynamic) 
			...
			if (true)
			{
				...
				for (int i = 0; i <= MAX_STEPS; i++)
				{
					...
					current_residual = abs(newSLAE.MSG_PreconditioningSSOR(MaxSize, needed_residual, Precond));
		...
		{
			std::cerr << "Error (MsFEM_forElasticDeformation_OrderBF1_forPoly): " << err << std::endl;
			int a;
			scanf_s("%d", &a);
		}
	};
